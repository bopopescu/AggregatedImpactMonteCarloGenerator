...
def selectProject():
	...
	if os.path.isdir(path+projectName.get()):
		infos=loadtxt(path+project_name+"\\info.csv", delimiter=",", dtype=str)
		...
		global iterations
		iterations=int(infos[1][25:])
		iterationNumber.destroy()
		iterationButton.destroy()
		Label(numberIterationFrame,text=int(infos[1][25:])).pack(side=LEFT)
...
def calculExecution():
	...
	system_filename = path+'programme\\export_7_3_2.csv' #export from Simapro
	CF_filenames = {}
	CF_filenames['IMPACT2002+'] = path+'programme\\IMPACT2002 update 2011.csv' #export from Simapro
	CF_filenames['IMPACT World midpoint'] = path+'programme\\IW+ 2012-09-05_Default_MidPt_aggreges_v0.01_no water quality.csv'
	CF_filenames['IMPACT World endpoint'] = path+'programme\\IW+ 2012-09-05_Default_EndPt_aggreges_v0.01_no water quality.csv'
	CF_filenames['Recipe midpoint'] = path+'programme\\recipe.csv'
	CF_filenames['Ecodex'] = path+'programme\\impact method ecodex.csv'
	...
	if correlatedMC.get():
		...
		if os.path.isdir(path+project_name+"\\correlated_impacts")==False: os.mkdir(path+project_name+"\\correlated_impacts")
		...
		MC(variables_technologique, variables_intervention, CF_matrix, CF_categories_name, iterations, UP_list, "all", path+project_name+"\\correlated_impacts/", [], progress)
		
		sigma_correlated, mu_correlated, sign_correlated=calcul_parameters(UP_list, path+project_name+"\\correlated_impacts\\")

		results_cor = csv.writer(open(path+project_name+"\\Monte-Carlo_results_correle.csv", "wb"))
		...
		if os.path.isdir(path+project_name+"\\variance_covariance")==False: os.mkdir(path+project_name+"\\variance_covariance")
		calcul_vcv(UP_list, impact_method, CF_categories_name, path+project_name+"/")
	...
	if nocorrelatedMC.get():
		...
		if os.path.isdir(path+project_name+"\\nocorrelated_impacts")==False: os.mkdir(path+project_name+"\\nocorrelated_impacts")
		if os.path.isdir(path+project_name+"\\trees")==False: os.mkdir(path+project_name+"\\trees")

		shutil.copyfile(path+"programme\\style.css", path+project_name+"\\trees\\style.css")
		shutil.copyfile(path+"programme\\fleche_bas.png", path+project_name+"\\trees\\fleche_bas.png")
		shutil.copyfile(path+"programme\\fleche_haut.png", path+project_name+"\\trees\\fleche_haut.png")
		shutil.copyfile(path+"programme\\script.js", path+project_name+"\\trees\\script.js")
		shutil.copyfile(path+"programme\\croix.png", path+project_name+"\\trees\\croix.png")
		
		processRunned=Label(informationsFrame,text="")
		processRunned.pack(side=LEFT)
		...
		for system_number in range(nocorrBegin.get(),nocorrEnd.get()): #disaggregation for every system
			...
			tree(UP_list_desag, UP_meta_info, impact_method, CF_categories, all_system_scores, all_unit_scores, CF_units, path+project_name+"\\trees\\")
			...
			MC(variables_technologique, variables_intervention, CF_transformed, CF_categories_name, iterations, UP_list_desag, system_number, path+project_name+"\\nocorrelated_impacts\\", systems, progress)
			
		sigma_nocorrelated, mu_nocorrelated, sign_nocorrelated=calcul_parameters(UP_list[:-4], path+project_name+"\\nocorrelated_impacts\\")
		results_nocor = csv.writer(open(path+project_name+"\\Monte-Carlo_results_nocorrele.csv", "wb"))
		for up in range(len(UP_list)-4):
			try:
				...
				results_nocor.writerow([up, UP_list[up]]+["-"]*len(CF_categories)*4)
...
if 1: 
	...
	###choix de la methode
	impact_methodes=['Ecodex','IMPACT2002+ midpoint', 'IMPACT2002+ endpoint', 'IMPACT World midpoint', 'IMPACT World endpoint', 'Recipe midpoint']
